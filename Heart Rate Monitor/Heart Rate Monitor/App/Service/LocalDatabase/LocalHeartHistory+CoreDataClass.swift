...
public class LocalHeartHistory: NSManagedObject {
    ...
    func toRemoteHistory() -> HeartRateHistory {
        HeartRateHistory(id: Int(self.id), grapValues: (self.grapValues ?? "").split(separator: ",").map { (Double($0) ?? 0.0) }, heartRateNumber: Int(self.heartRateNumber), label: self.label, createDate: self.createDate, isSubmitted: self.isSubmitted)
    }
}
