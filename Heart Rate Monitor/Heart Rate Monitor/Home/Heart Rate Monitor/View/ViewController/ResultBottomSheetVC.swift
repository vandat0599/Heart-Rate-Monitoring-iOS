...
class ResultBottomSheetVC: BottomSheetViewController {
    ...
    @objc private func rightActionTapped() {
        var history = HeartRateHistory(id: nil, grapValues: self.grapsValues, heartRateNumber: self.heartRate, label: self.labelTextField.text, createDate: String(Int(Date().timeIntervalSince1970) ?? 0), isSubmitted: false)
        print("saved: \(history)")
